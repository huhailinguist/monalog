...
class P2A_transformer:
    def __init__(self, nlp_spacy):
        self.nlp = nlp_spacy  # spacy.load('en')

        self.noundict = {'i':'me', 'we':'us', 'you':'you', 'he':'him', 'she':'her', 'they':'them', 'them':'they', 'her':'she', 'him':'he', 'us':'we', 'me':'i'}

    def nouninv(self, noun):
        n = noun.lower()
        if n in self.noundict:
            return self.noundict[n]
        return noun
    ...
    def pass2act(self, doc, rec=False):
        ...
        for sent in parse.sents:
            ...
            aplural = False
            advcltree = None
            aux = list(list(self.nlp('. .').sents)[0]) # start with 2 'null' elements
            xcomp = ''
            punc = '.'
            ...
            # invert nouns:
            agent = self.nouninv(agent)
            subjpass = self.nouninv(subjpass)
